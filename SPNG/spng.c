...
static unsigned num_channels(const struct spng_ihdr* ihdr)
{
	...
	{ return 1; } /* grayscale or indexed color */
}
...
static int calculate_image_size(spng_ctx* ctx, int fmt, size_t* len)
{
	...
	if (fmt & ((unsigned)fmt - 1))
	{ return SPNG_EFMT; } /* More than one bit is set */
...
static int increase_cache_usage(spng_ctx* ctx, size_t bytes, int new_chunk)
{
	...
	if (new_chunk)
	{
		...
		if (ctx->chunk_count_total < 1)
		{ return SPNG_EOVERFLOW; }

		if (ctx->chunk_count_total > ctx->chunk_count_limit)
		{ return SPNG_ECHUNK_LIMITS; }
	}

	size_t new_usage = ctx->chunk_cache_usage + bytes;

	if (new_usage < ctx->chunk_cache_usage)
	{ return SPNG_EOVERFLOW; }

	if (new_usage > ctx->chunk_cache_limit)
	{ return SPNG_ECHUNK_LIMITS; }
...
static inline int read_header(spng_ctx* ctx)
{
	...
	if (is_critical_chunk(&chunk) && ctx->crc_action_critical == SPNG_CRC_USE)
	{ ctx->skip_crc = 1; }
...
static int read_chunk_bytes(spng_ctx* ctx, uint32_t bytes)
{
	if (ctx == NULL)
	{ return SPNG_EINTERNAL; }
	if (!ctx->cur_chunk_bytes_left || !bytes)
	{ return SPNG_EINTERNAL; }
	if (bytes > ctx->cur_chunk_bytes_left)
	{ return SPNG_EINTERNAL; } /* XXX: more specific error? */
...
static int spng__inflate_init(spng_ctx* ctx, int window_bits)
{
	...
	if (inflateInit2(&ctx->zstream, window_bits) != Z_OK)
	...
	if (ctx->flags & SPNG_CTX_IGNORE_ADLER32)
	{ validate = 0; }
	...
	if (inflateValidate(&ctx->zstream, validate))
	{ return SPNG_EZLIB_INIT; }
...
static int spng__deflate_init(spng_ctx* ctx, struct spng__zlib_options* options)
{
	...
	int ret = deflateInit2(zstream, options->compression_level, Z_DEFLATED, options->window_bits, options->mem_level,
			options->strategy);
...
*/
static int
spng__inflate_stream(spng_ctx* ctx, char** out, size_t* len, size_t extra, const void* start_buf, size_t start_len)
{
	...
	while (ret != Z_STREAM_END)
	{
		...
		else if (!stream->avail_in) /* Read more chunk bytes */
		{
			...
			if (ctx->streaming && read_size > SPNG_READ_SIZE)
			{ read_size = SPNG_READ_SIZE; }
...
static int read_idat_bytes(spng_ctx* ctx, uint32_t* bytes_read)
{
	...
	{ return SPNG_EINTERNAL; }
	if (memcmp(ctx->current_chunk.type, type_idat, 4))
	{ return SPNG_EIDAT_TOO_SHORT; }
	...
	{/* TODO: estimate bytes to read for progressive reads */
		len = SPNG_READ_SIZE;
		if (len > ctx->cur_chunk_bytes_left)
		{ len = ctx->cur_chunk_bytes_left; }
	}
	else
	{ len = ctx->current_chunk.length; }
...
{
	if (prev_scanline == NULL || scanline == NULL || !scanline_width)
	{ return SPNG_EINTERNAL; }
...
static void
unpack_scanline(unsigned char* out, const unsigned char* scanline, uint32_t width, unsigned bit_depth, int fmt)
{
...
{
	if (!memcmp(type, type_plte, 4))
	{ return 1; }
	else if (!memcmp(type, type_chrm, 4))
	{ return 1; }
	else if (!memcmp(type, type_gama, 4))
	{ return 1; }
	else if (!memcmp(type, type_sbit, 4))
	{ return 1; }
	else if (!memcmp(type, type_srgb, 4))
	{ return 1; }
	else if (!memcmp(type, type_bkgd, 4))
	{ return 1; }
	else if (!memcmp(type, type_trns, 4))
	{ return 1; }
	else if (!memcmp(type, type_hist, 4))
	{ return 1; }
	else if (!memcmp(type, type_phys, 4))
	{ return 1; }
	else if (!memcmp(type, type_time, 4))
	{ return 1; }
	else if (!memcmp(type, type_offs, 4))
	{ return 1; }
	else
	{ return 0; }
}
...
static int read_ihdr(spng_ctx* ctx)
{
	...
	if (memcmp(data, spng_signature, sizeof(spng_signature)))
	{ return SPNG_ESIGNATURE; }

	chunk->length = read_u32(data + 8);
	memcpy(&chunk->type, data + 12, 4);

	if (chunk->length != 13)
	{ return SPNG_EIHDR_SIZE; }
	...
	{ ctx->bytes_per_pixel = ctx->channels * (ctx->ihdr.bit_depth / 8); }
...
static int read_non_idat_chunks(spng_ctx* ctx)
{
	...
	while (!(ret = read_header(ctx)))
	{
		...
		if (!memcmp(chunk.type, type_idat, 4))
		{
			...
			{
				if (ctx->ihdr.color_type == 3 && !ctx->stored.plte)
				{ return SPNG_ENOPLTE; }
			...
			{ return SPNG_ECHUNK_POS; } /* IDAT chunk not at the end of the IDAT sequence */
		}
		...
		if (is_critical_chunk(&chunk))
		{
			...
			{
				if (ctx->file.trns || ctx->file.hist || ctx->file.bkgd)
				{ return SPNG_ECHUNK_POS; }
				if (chunk.length % 3 != 0)
				{ return SPNG_ECHUNK_SIZE; }

				ctx->plte.n_entries = chunk.length / 3;

				if (check_plte(&ctx->plte, &ctx->ihdr))
				{ return SPNG_ECHUNK_SIZE; } /* XXX: EPLTE? */
			...
			else if (!memcmp(chunk.type, type_ihdr, 4))
			{ return SPNG_ECHUNK_POS; }
			else
			{ return SPNG_ECHUNK_UNKNOWN_CRITICAL; }
		}
		else if (!memcmp(chunk.type, type_chrm, 4)) /* Ancillary chunks */
		{
			...
			if (check_chrm_int(&ctx->chrm_int))
			{ return SPNG_ECHRM; }
		...
		else if (!memcmp(chunk.type, type_sbit, 4))
		{
			...
			if (check_sbit(&ctx->sbit, &ctx->ihdr))
			{ return SPNG_ESBIT; }
		...
		else if (!memcmp(chunk.type, type_srgb, 4))
		{
			...
			if (ctx->srgb_rendering_intent > 3)
			{ return SPNG_ESRGB; }
		...
		else if (!memcmp(chunk.type, type_bkgd, 4))
		{
			...
			else if (ctx->ihdr.color_type == 3)
			{
				...
				if (ctx->bkgd.plte_index >= ctx->plte.n_entries)
		...
		else if (!memcmp(chunk.type, type_hist, 4))
		{
			...
			{ return SPNG_ECHUNK_POS; }
			if (ctx->file.hist)
			{ return SPNG_EDUP_HIST; }

			if ((chunk.length / 2) != (ctx->plte.n_entries))
			{ return SPNG_ECHUNK_SIZE; }
		...
		else /* Arbitrary-length chunk */
		{
			...
			if (!memcmp(chunk.type, type_exif, 4))
			{
				...
				if (increase_cache_usage(ctx, chunk.length, 1))
				{ return SPNG_ECHUNK_LIMITS; }
			...
			{/* TODO: add test file with color profile */
				...
				if (check_png_keyword(ctx->iccp.profile_name))
				{ return SPNG_EICCP_NAME; }

				if (chunk.length < (keyword_len + 2))
				{ return SPNG_ECHUNK_SIZE; }

				if (ctx->data[keyword_len + 1] != 0)
				{ return SPNG_EICCP_COMPRESSION_METHOD; }
					 ...
					 !memcmp(chunk.type, type_itxt, 4))
			{
				...
				if (increase_cache_usage(ctx, sizeof(struct spng_text2), 1))
				{ return SPNG_ECHUNK_LIMITS; }

				ctx->n_text++;
				if (ctx->n_text < 1)
				{ return SPNG_EOVERFLOW; }
				if (sizeof(struct spng_text2) > SIZE_MAX / ctx->n_text)
				{ return SPNG_EOVERFLOW; }
				...
				if (peek_bytes > chunk.length)
				{ peek_bytes = chunk.length; }
				...
				else if (!memcmp(chunk.type, type_itxt, 4))
				{
					...
					if ((peek_bytes - keyword_len) < 5)
					{ return SPNG_EITXT; }

					text->compression_flag = keyword_nul[1];

					if (text->compression_flag > 1)
					...
					const unsigned char* zlib_stream = memchr(data + translated_keyword_offset, 0,
							peek_bytes - translated_keyword_offset);
					if (zlib_stream == NULL)
					{ return SPNG_EITXT; }
					if (zlib_stream == peek_end)
					{ return SPNG_EITXT; }
				...
				if (text->compression_flag)
				{
					...
					if (increase_cache_usage(ctx, peek_bytes, 0))
					{ return SPNG_ECHUNK_LIMITS; }
				...
				{
					if (increase_cache_usage(ctx, chunk.length + 1, 0))
					{ return SPNG_ECHUNK_LIMITS; }
				...
				if (check_png_keyword(text->keyword))
				{ return SPNG_ETEXT_KEYWORD; }
			...
			else if (!memcmp(chunk.type, type_splt, 4))
			{
				...
				{ return SPNG_ECHUNK_POS; }
				if (ctx->user.splt)
				{ goto discard; } /* XXX: could check profile names for uniqueness */
			...
			else /* Unknown chunk */
			{
				...
				if (increase_cache_usage(ctx, chunk.length + sizeof(struct spng_unknown_chunk), 1))
				{
					return SPNG_ECHUNK_LIMITS;
				}
				...
				if (ctx->n_chunks < 1)
				{ return SPNG_EOVERFLOW; }
				if (sizeof(struct spng_unknown_chunk) > SIZE_MAX / ctx->n_chunks)
				{ return SPNG_EOVERFLOW; }
				...
				{
					if (ctx->file.plte)
					{ chunkp->location = SPNG_AFTER_PLTE; }
					else
					{ chunkp->location = SPNG_AFTER_IHDR; }
				}
				else if (ctx->state >= SPNG_STATE_AFTER_IDAT)
				{ chunkp->location = SPNG_AFTER_IDAT; }
...
static int read_scanline(spng_ctx* ctx)
{
	...
	if (ctx->ihdr.bit_depth == 16 && ctx->fmt != SPNG_FMT_RAW)
...
int spng_decode_scanline(spng_ctx* ctx, void* out, size_t len)
{
	...
	for (k = 0; k < width; k++)
	{
		...
		else if (ihdr->color_type == SPNG_COLOR_TYPE_GRAYSCALE)
		{
			if (ihdr->bit_depth == 16)
			{
				...
				if (f.apply_trns && ctx->trns.gray == gray_16)
				{ a_16 = 0; }
				else
				{ a_16 = 65535; }
			...
			else /* <= 8 */
			{
				...
				if (f.apply_trns && ctx->trns.gray == gray_8)
				{ a_8 = 0; }
				else
				{ a_8 = 255; }
	...
	if (f.apply_trns)
	{ trns_row(out, scanline, trns_px, ctx->bytes_per_pixel, &ctx->ihdr, width, fmt); }

	if (f.do_scaling)
	{ scale_row(out, width, fmt, processing_depth, sb); }

	if (f.apply_gamma)
	{ gamma_correct_row(out, width, fmt, gamma_lut); }
...
int spng_decode_row(spng_ctx* ctx, void* out, size_t len)
{
	...
	if (!ihdr->interlace_method || pass == 6)
	{ return spng_decode_scanline(ctx, out, len); }
	...
	if (ctx->fmt == SPNG_FMT_RGBA16)
	{ pixel_size = 8; }
	else if (ctx->fmt == SPNG_FMT_RGB8)
	{ pixel_size = 3; }
	else if (ctx->fmt == SPNG_FMT_G8)
	{ pixel_size = 1; }
	else if (ctx->fmt == SPNG_FMT_GA8)
	{ pixel_size = 2; }
	else if (ctx->fmt & (SPNG_FMT_PNG | SPNG_FMT_RAW))
	{
		...
		{ pixel_size = ctx->bytes_per_pixel; }
	}
...
int spng_decode_image(spng_ctx* ctx, void* out, size_t len, int fmt, int flags)
{
	...
	if (flags & SPNG_DECODE_TRNS && ctx->stored.trns)
	{ f.apply_trns = 1; }
	else
	...
	if (flags & SPNG_DECODE_USE_SBIT && ctx->stored.sbit)
	{ f.use_sbit = 1; }
	else
	...
	if (f.apply_gamma)
	{
		...
		{ return decode_err(ctx, SPNG_EGAMA); }
	...
	{
		if (!sub[i].scanline_width)
		{ continue; }

		if (fmt & (SPNG_FMT_PNG | SPNG_FMT_RAW))
		{ sub[i].out_width = sub[i].scanline_width - 1; }
		else
		{ sub[i].out_width = (size_t)sub[i].width * pixel_size; }

		if (sub[i].out_width > UINT32_MAX)
		{ return decode_err(ctx, SPNG_EOVERFLOW); }
	}
	...
	if (ri->filter > 4)
	{ return decode_err(ctx, SPNG_EFILTER); }
...
static int encode_scanline(spng_ctx* ctx, const void* scanline, size_t len)
{
	...
	const int requires_previous =
			f.filter_choice & (SPNG_FILTER_CHOICE_UP | SPNG_FILTER_CHOICE_AVG | SPNG_FILTER_CHOICE_PAETH);
	...
	if (filter)
	{
		ret = filter_scanline(filtered_scanline, ctx->prev_scanline, ctx->scanline, scanline_width,
				ctx->bytes_per_pixel, filter);
		if (ret)
		{ return encode_err(ctx, ret); }
	}
...
int spng_encode_image(spng_ctx* ctx, const void* img, size_t len, int fmt, int flags)
{
	...
	{ ctx->bytes_per_pixel = ctx->channels * (ihdr->bit_depth / 8); }
...
int spng_set_png_buffer(spng_ctx* ctx, const void* buf, size_t size)
{
	...
	{ return SPNG_EBADSTATE; }
	if (ctx->encode_only)
	{ return SPNG_ECTXTYPE; } /* not supported */
...
int spng_set_crc_action(spng_ctx* ctx, int critical, int ancillary)
{
	...
	if (critical > 2 || critical < 0)
	{ return 1; }
	if (ancillary > 2 || ancillary < 0)
	{ return 1; }
...
int spng_set_iccp(spng_ctx* ctx, struct spng_iccp* iccp)
{
	...
	{ return SPNG_EICCP_NAME; }
	if (!iccp->profile_len || iccp->profile_len > UINT_MAX)
	{ return 1; }
