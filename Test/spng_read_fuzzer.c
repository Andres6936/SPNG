...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
	...
	if (stream)
	{
		...
		{ ret = spng_set_png_stream(ctx, buffer_read_fn, &state); }
	}
	else
	{ ret = spng_set_png_buffer(ctx, (void*)data, size); }
	...
	if (progressive)
	{
		if (spng_decode_image(ctx, NULL, 0, fmt, flags | SPNG_DECODE_PROGRESSIVE))
		{ goto err; }
	...
	else if (spng_decode_image(ctx, out, out_size, fmt, flags))
	{ goto err; }
