...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
	...
	if (stream)
	{ ret = spng_set_png_stream(ctx, stream_write_fn, &state); }
	else
	...
	if (progressive)
	{
		if (spng_encode_image(ctx, NULL, 0, fmt, SPNG_ENCODE_PROGRESSIVE | SPNG_ENCODE_FINALIZE))
		{ goto err; }
	...
	else if (spng_encode_image(ctx, img, img_size, fmt, SPNG_ENCODE_FINALIZE))
	{ goto err; }
