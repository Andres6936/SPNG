...
static void print_test_args(struct spngt_test_case* test_case)
{
	...
	if (!test_case->flags && !test_case->test_flags)
	{ printf("(NONE)"); }

	if (test_case->flags & SPNG_DECODE_TRNS)
	{ printf("TRNS "); }
	if (test_case->flags & SPNG_DECODE_GAMMA)
	{ printf("GAMMA "); }

	if (test_case->test_flags & SPNGT_COMPARE_CHUNKS)
	{ printf("COMPARE_CHUNKS "); }

	if (test_case->test_flags & SPNGT_EXTENDED_TESTS)
	{ printf("EXTENDED "); }

	if (test_case->test_flags & SPNGT_SKIP)
	{ printf("[SKIPPED]"); }
		...
		size_t img_size)
{
	...
	if (fmt & (SPNG_FMT_PNG | SPNG_FMT_RAW))
	{
		...
		{/* gray 1,2,4,8,16 bits, indexed 1,2,4,8 or gray alpha 8,16  */
			...
			{ bytes_per_pixel = channels * (ihdr->bit_depth / 8); }
...
static void print_chunks(spngt_chunk_bitfield chunks)
{
	...
	if (!memcmp(&none, &chunks, sizeof(spngt_chunk_bitfield)))
	{ printf(" (none)"); }
...
static int get_chunks(spng_ctx* ctx, spngt_chunk_data* spng)
{
	...
	else if (ret == SPNG_ECHUNKAVAIL)
	{ ret = 0; }
	else
	{ return ret; }

	if (!spng_get_trns(ctx, &spng->trns))
	{ spng->have.trns = 1; }
	if (!spng_get_chrm(ctx, &spng->chrm))
	{ spng->have.chrm = 1; }
	if (spng->have.chrm)
	{ spng_get_chrm_int(ctx, &spng->chrm_int); }
	if (!spng_get_gama(ctx, &spng->gamma))
	{ spng->have.gama = 1; }
	if (spng->have.gama)
	{ spng_get_gama_int(ctx, &spng->gamma_int); }
	if (!spng_get_iccp(ctx, &spng->iccp))
	{ spng->have.iccp = 1; }
	if (!spng_get_sbit(ctx, &spng->sbit))
	{ spng->have.sbit = 1; }
	if (!spng_get_srgb(ctx, &spng->srgb_rendering_intent))
	{ spng->have.srgb = 1; }
	if (!spng_get_text(ctx, NULL, &spng->n_text))
	{ spng->have.text = 1; }
	if (!spng_get_bkgd(ctx, &spng->bkgd))
	{ spng->have.bkgd = 1; }
	if (!spng_get_hist(ctx, &spng->hist))
	{ spng->have.hist = 1; }
	if (!spng_get_phys(ctx, &spng->phys))
	{ spng->have.phys = 1; }
	if (!spng_get_splt(ctx, NULL, &spng->n_splt))
	{ spng->have.splt = 1; }
	if (!spng_get_time(ctx, &spng->time))
	{ spng->have.time = 1; }
	if (!spng_get_offs(ctx, &spng->offs))
	{ spng->have.offs = 1; }
	if (!spng_get_exif(ctx, &spng->exif))
	{ spng->have.exif = 1; }
	if (!spng_get_unknown_chunks(ctx, NULL, &spng->n_unknown_chunks))
	{ spng->have.unknown = 1; }
...
static int set_chunks(spng_ctx* dst, spngt_chunk_data* spng)
{
	...
	if (spng->have.plte)
	{ spng_set_plte(dst, &spng->plte); }
	if (spng->have.trns)
	{ spng_set_trns(dst, &spng->trns); }
	if (spng->have.chrm)
	{ spng_set_chrm_int(dst, &spng->chrm_int); }
	if (spng->have.gama)
	{ spng_set_gama_int(dst, spng->gamma_int); }
	if (spng->have.iccp)
	{ spng_set_iccp(dst, &spng->iccp); }
	if (spng->have.sbit)
	{ spng_set_sbit(dst, &spng->sbit); }
	if (spng->have.srgb)
	{ spng_set_srgb(dst, spng->srgb_rendering_intent); }
	if (spng->have.text)
	{ spng_set_text(dst, spng->text, spng->n_text); }
	if (spng->have.bkgd)
	{ spng_set_bkgd(dst, &spng->bkgd); }
	if (spng->have.hist)
	{ spng_set_hist(dst, &spng->hist); }
	if (spng->have.phys)
	{ spng_set_phys(dst, &spng->phys); }
	if (spng->have.splt)
	{ spng_set_splt(dst, spng->splt, spng->n_splt); }
	if (spng->have.time)
	{ spng_set_time(dst, &spng->time); }
	if (spng->have.offs)
	{ spng_set_offs(dst, &spng->offs); }
	if (spng->have.exif)
	{ spng_set_exif(dst, &spng->exif); }
	if (spng->have.unknown)
	{ spng_set_unknown_chunks(dst, spng->chunks, spng->n_unknown_chunks); }
...
static int compare_chunks(spng_ctx* ctx, png_infop info_ptr, png_structp png_ptr, int after_idat)
{
	...
	if (png_get_PLTE(png_ptr, info_ptr, &png_palette, &png_num_palette) == PNG_INFO_PLTE)
	{
		png.n_plte_entries = png_num_palette;
	}
	...
	if (spng.have.iccp)
	{
		...
		png_get_iCCP(png_ptr, info_ptr, &png_iccp_name, &png_iccp_compression_type, &png_iccp_profile,
				&png_iccp_proflen);
	...
	if (spng.have.bkgd)
	{
		...
		if (spng.ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE ||
			spng.ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE_ALPHA)
		{
			if (spng.bkgd.gray != png_bkgd->gray)
			{
				...
				ret = 1;
				 ...
				 spng.ihdr.color_type == SPNG_COLOR_TYPE_TRUECOLOR_ALPHA)
		{
	...
	if (spng.have.phys)
	{
		uint32_t png_phys_res_x, png_phys_rex_y;
		int png_phys_unit_type;
...
static int decode_and_compare(spngt_test_case* spng, spngt_test_case* png)
{
	...
	if (!memcmp(img_spng, img_png, img_spng_size))
	{ goto identical; }
...
cleanup:
	...
	if (spng.source.file)
	{ fclose(spng.source.file); }
	if (libpng.source.file)
	{ fclose(libpng.source.file); }
...
*/
	...
	if (n_test_cases != actual_count)
	{ printf(" (skipping %d)", n_test_cases - actual_count); }
	...
	for (i = 0; i < n_test_cases; i++)
	{
		...
		if (test_cases[i].test_flags & SPNGT_SKIP)
		{ continue; }
...
static int extended_tests(FILE* file, int fmt)
{
	...
	{ ret = 0; } /* clear the (expected) error */
